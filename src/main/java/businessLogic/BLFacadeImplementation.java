...
public class BLFacadeImplementation  implements BLFacade {
	...
	public void apustuaEgin(String erab, float price, Result res) throws NotEnoughMoney, betMinimum, EventFinished {
		...
		//DataAccess dBManager = new DataAccess();
		// bi kontainer erabili beharko dira atzera egiteko
		if(new Date().compareTo(dbManager.getQuestionContainer(dbManager.getResultContainer(res).getQuestion()).getEvent().getEventDate())>0)
			throw new EventFinished(ResourceBundle.getBundle(ETIQUETAS).getString("ErrorEventHasFinished"));
		...
		if(dbManager.getResultContainer(result).getQuestion().getBetMinimum()>price) throw new betMinimum(ResourceBundle.getBundle(ETIQUETAS).getString("betMinimum"));
